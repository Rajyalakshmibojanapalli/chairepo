...
const Header = ({ currentPage, navigateTo, mobileMenuOpen, toggleMobileMenu }) => {
  const [scrollY, setScrollY] = useState(0);
  ...
  return (
    <header className={headerClasses} style={gradientStyle}>
      <div className="w-full mx-auto">
        {/* Desktop Navigation */}
        <nav className="hidden md:flex justify-center items-center h-full text-white text-center mx-auto relative">
          {leftNavItems.map((item, index) => (
            <a
              key={index}
              href={`#${item.link}`}
              ...
              className={`mx-3 font-medium transition-colors ${
                currentPage === item.link ? "text-[#8dcb3f]" : "text-white hover:text-[#8dcb3f]"
              }`}
            >
              {item.title}
          ...
          <div className="mx-10">
            <img
              src={icon}
              alt="Logo"
              className={`transition-all duration-300 ${isSmaller ? "h-[45px]" : "h-[105px]"}`}
              onClick={() => navigateTo("home")}
              style={{ cursor: "pointer" }}
            />
          </div>
          ...
          <div className="md:hidden absolute top-full left-0 w-full bg-[#d16b02] shadow-lg z-50">
            <ul className="flex flex-col w-full">
              {navItems.map((item, index) => (
                <li
                  key={index}
                  className="w-full border-b border-[#8dcb3f] border-opacity-20"
                >
                  <a
                    href={`#${item.link}`}
                    onClick={(e) => {
                    ...
                    className={`block font-medium py-4 px-6 transition-colors ${
                      currentPage === item.link
                        ? "text-[#8dcb3f]"
                        : "text-white hover:text-[#8dcb3f]"
                    }`}
                  >
                    {item.title}
                  </a>
